...
func (p *Player) SetSkin(skin skin.Skin) {
	...
	if p.Handle(func(h Handler) *event.Context {
...
// Handle calls the provided function on each handler. This function returns false if
// the event is cancelled or not.
func (p *Player) Handle(f func(h Handler) *event.Context) bool {
	cancelled := false

	p.h.Range(func(key, value any) bool {
		h := value.(Handler)
...
func (p *Player) RemoveHandler(key string) {
	p.h.Delete(key)
}
...
func (p *Player) ExecuteCommand(commandLine string) {
	...
	ctx := event.C()
	if p.Handle(func(h Handler) *event.Context {
		h.HandleCommandExecution(ctx, command, args[1:])
		return ctx
	}) {
		return
	}
...
func (p *Player) Heal(health float64, source world.HealingSource) {
	...
	if p.Handle(func(h Handler) *event.Context {
		h.HandleHeal(ctx, &health, source)
		return ctx
	}) {
		return
	}
...
func (p *Player) Hurt(dmg float64, src world.DamageSource) (float64, bool) {
	...
	if p.Handle(func(h Handler) *event.Context {
		h.HandleHurt(ctx, &dmg, &immunity, src)
		return ctx
	}) {
		return 0, false
	}
...
func (p *Player) Exhaust(points float64) {
	...
	if after := p.hunger.Food(); before != after {
		...
		ctx := event.C()
		if p.Handle(func(h Handler) *event.Context {
			h.HandleFoodLoss(ctx, before, &after)
			ctx.Cancelled()
			return ctx
		}) {
			return
...
func (p *Player) StopSprinting() {
	ctx := event.C()
	if p.Handle(func(h Handler) *event.Context {
		h.HandleToggleSprint(ctx, false)
		return ctx
	}) {
		return
	}
...
func (p *Player) drops(held item.Stack, pos cube.Pos, w *world.World) []item.Stack {
...
func (p *Player) Tick(w *world.World, current int64) {
	...
	if p.lastTickedWorld != w {
		p.Handle(
			func(h Handler) *event.Context {
				h.HandleChangeWorld(p.lastTickedWorld, w)
				return nil
			},
		)
	}
...
func (p *Player) EditSign(pos cube.Pos, frontText, backText string) error {
	...
	if frontText != sign.Front.Text {
		if p.Handle(func(h Handler) *event.Context {
			h.HandleSignEdit(ctx, true, sign.Front.Text, frontText)
			ctx.Cancelled()
			return ctx
		}) {
			return nil
		...
		if p.Handle(func(h Handler) *event.Context {
			h.HandleSignEdit(ctx, false, sign.Back.Text, backText)
			ctx.Cancelled()
			return ctx
		}) {
			return nil
...
func (p *Player) TurnLecternPage(pos cube.Pos, page int) error {
	...
	ctx := event.C()
	if p.Handle(func(h Handler) *event.Context {
		h.HandleLecternPageTurn(ctx, pos, lectern.Page, &page)
		return ctx
	}) {
		return nil
	}
